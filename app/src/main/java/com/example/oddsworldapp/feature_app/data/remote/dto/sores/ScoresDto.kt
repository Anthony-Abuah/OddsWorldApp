package com.example.oddsworldapp.feature_app.data.remote.dto.sores

import com.example.oddsworldapp.feature_app.data.local.entities.scores_entity.ScoresEntity
import com.example.oddsworldapp.feature_app.domain.model.scores.Scores

data class ScoresDto(
    val away_team: String,
    val commence_time: String,
    val completed: Boolean,
    val home_team: String,
    val id: String,
    val last_update: String,
    val scores: List<ScoreDto>?,
    val sport_key: String,
    val sport_title: String
){

    fun toScores(): Scores{
        return Scores(away_team, commence_time, completed, home_team, id, last_update, scores?.map { it.toScore() } , sport_key, sport_title)
    }
    fun toScoresEntity(): ScoresEntity{
        return ScoresEntity(null,away_team, commence_time, completed, home_team, id, last_update, scores?.map { it.toScore() } , sport_key, sport_title)
    }
}